 ...
 *
 * The difference between mlp.cpp and this file is that it
 * checks the MXSymbolGrad function in forward case (for now).
 */
...
void MLP() {
  ...
  SymbolHandle hGrad;
  const char *wrt[] = { "w0" };
  CHECK_EQ(MXSymbolGrad(sym_out.GetHandle(), 1, wrt, &hGrad), 0);
  Symbol grad(hGrad);

  std::vector<Symbol> outs {sym_out, grad};
  Symbol out_group = Symbol::Group(outs);
  ...
  // We don't compute the gradients with dL/dw1 in forward graph since
  // the backward function for FullyConnected does not have a gradient
  // registered.  I'm considering to restruct this thing so that we don't
  // rely on dedicated backward ops for every single type of layers in this
  // case, otherwise we will need to have a backward for
  // "_backward_FullyConnected", and potentially another for
  // "_backward_backward_FullyConnected", etc., which is a disaster.
  std::vector<NDArray> arg_group_grad_store;
  std::vector<OpReqType> group_grad_req_type;
  for (int i = 0; i < 6; ++i) {
    arg_group_grad_store.push_back(NDArray());
    group_grad_req_type.push_back(kNullOp);
  }

  cout << "make the Executor" << endl;
  Executor* exe_group = new Executor(out_group, ctx_dev, in_args, arg_group_grad_store,
                                     group_grad_req_type, aux_states);
  ...
  for (int iter = 0; iter < max_iters; ++iter) {
    exe_group->Forward(true);

    std::vector<NDArray>& out = exe_group->outputs;
    float* cptr = new float[128 * 10];
    float* gptr = new float[512 * 28];
    float* gptr1 = new float[512 * 28];
    out[0].SyncCopyToCPU(cptr, 128 * 10);
    out[1].SyncCopyToCPU(gptr, 512 * 28);
    ...
    arg_grad_store[1].SyncCopyToCPU(gptr1, 512 * 28);
    NDArray::WaitAll();
    // These must be equal as gptr and gptr1 both computes dL/dw1.
    CHECK_EQ(gptr[0], gptr1[0]);
    CHECK_EQ(gptr[3], gptr1[3]);
    delete[] cptr;
    delete[] gptr;
    delete[] gptr1;
  }
